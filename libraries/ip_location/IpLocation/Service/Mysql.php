<?php
/**
 * This file contains the class KbIpLocation_Service_Mysql.
 * 
 * PHP Version 5.0.0
 *
 * @package IpLocation
 ...
 *
 */
class KbIpLocation_Service_Mysql extends KbIpLocation_Service_Abstract
{
    /**
    ...
    /**
     * KbIpLocation_Service_Mysql
     *
     * @param array $options An array of database connection options.
     */
    public function KbIpLocation_Service_Mysql($options)
    {
     ...
     * KbIpLocation_Results object containing the results. If any results are not
     * found or if any problem occured then false is returned.
     *
     * @param string $ip The ip address to lookup.
     *
     * @return boolean|KbIpLocation_Results The location in the form of an
     *                                    KbIpLocation_Results object. False
    ...
    public function getIpLocation($ip)
    {
        ...
        WHERE ipFrom <= " . pSQL($convertedIp) . " 
        AND ipTo >=" . pSQL($convertedIp) . ";";
        ...
        if (($result && mysql_num_rows($result) >= 0)) {
            ...
            while ($row = mysql_fetch_assoc($result)) {
                return new KbIpLocation_Results(
                    $ip, 
                    $row['country2Char'], 
                    $row['countryName']
                );
