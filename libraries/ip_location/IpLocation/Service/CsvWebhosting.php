<?php
/**
 * This file contains the class KbIpLocation_Service_CsvWebhosting.
 *
 * PHP Version 5.0.0
 *
 * @package IpLocation
 ...
 *
 */
class KbIpLocation_Service_CsvWebhosting extends KbIpLocation_Service_Abstract
{
    /**
     ...
     *
     * @return string The location as a KbIpLocation_Results object
     */
        ...
        if (($handle = fopen(dirname(__FILE__) . '/data/' . $this->ipCsvFile, "r")) !== false) {
            while (($data = fgetcsv($handle, 1000, ",")) !== false) {
                if ($convertedIp > $data[0] && $convertedIp < $data[1]) {
                    return new KbIpLocation_Results($ip, $data[2], $data[4]);
                }
            }
            fclose($handle);
        }
